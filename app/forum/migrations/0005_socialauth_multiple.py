...
FACEBOOK_ORIGINAL_BACKEND_NAME = 'facebook'
FACEBOOK_APP = settings.djsettings.FACEBOOK_APP
FACEBOOK_OVERRIDE_BACKEND_NAME = getattr(settings.djsettings, 'FACEBOOK_OVERRIDE_BACKEND_NAME', None)

LINKEDIN_ORIGINAL_BACKEND_NAME = 'linkedin-oauth2'
LINKEDIN_APP = settings.djsettings.LINKEDIN_APP
LINKEDIN_OVERRIDE_BACKEND_NAME = getattr(settings.djsettings, 'LINKEDIN_OVERRIDE_BACKEND_NAME', None)
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        # Obtain models
        User = orm['forum.User']
        FacebookAccount = orm['forum.FacebookAccount']
        LinkedinAccount = orm['forum.LinkedinAccount']

        # Migrate Facebook data
        for user in User.objects.filter(facebook_access_token__isnull=False):
            FacebookAccount(app=FACEBOOK_APP, user=user,
                            uid=user.facebook_uid,
                            email=user.facebook_email,
                            access_token=user.facebook_access_token,
                            access_token_expires_on=user.facebook_access_token_expires_on).save()

        # Update Facebook backend name
        if FACEBOOK_OVERRIDE_BACKEND_NAME:
            for usersocialauth in UserSocialAuth.objects.filter(provider=FACEBOOK_ORIGINAL_BACKEND_NAME):
                usersocialauth.provider = FACEBOOK_OVERRIDE_BACKEND_NAME
                usersocialauth.save()

        # Migrate Linkedin data
        for user in User.objects.filter(linkedin_access_token__isnull=False):
            LinkedinAccount(app=LINKEDIN_APP, user=user,
                            uid=user.linkedin_uid,
                            email=user.linkedin_email,
                            access_token=user.linkedin_access_token,
                            access_token_expires_on=user.linkedin_access_token_expires_on,
                            photo_url=user.linkedin_photo_url).save()

        # Update Linkedin backend name
        if LINKEDIN_OVERRIDE_BACKEND_NAME:
            for usersocialauth in UserSocialAuth.objects.filter(provider=LINKEDIN_ORIGINAL_BACKEND_NAME):
                usersocialauth.provider = LINKEDIN_OVERRIDE_BACKEND_NAME
                usersocialauth.save()
    ...
    def backwards(self, orm):
        ...
        # Migrate Facebook data
        for user in User.objects.filter(facebook_accounts__isnull=False):
            account = user.facebook_accounts.get(app=FACEBOOK_APP)

            user.facebook_uid = account.uid
            user.facebook_email = account.email
            user.facebook_access_token = account.access_token
            user.facebook_access_token_expires_on = account.access_token_expires_on
            user.save()

        # Update Facebook backend name
        if FACEBOOK_OVERRIDE_BACKEND_NAME:
            for usersocialauth in UserSocialAuth.objects.filter(provider=FACEBOOK_OVERRIDE_BACKEND_NAME):
                usersocialauth.provider = FACEBOOK_ORIGINAL_BACKEND_NAME
                usersocialauth.save()

        # Migrate Linkedin data
        for user in User.objects.filter(linkedin_accounts__isnull=False):
            account = user.linkedin_accounts.get(app=LINKEDIN_APP)

            user.linkedin_uid = account.uid
            user.linkedin_email = account.email
            user.linkedin_access_token = account.access_token
            user.linkedin_access_token_expires_on = account.access_token_expires_on
            user.linkedin_photo_url = account.photo_url
            user.save()

        # Update Linkedin backend name
        if LINKEDIN_OVERRIDE_BACKEND_NAME:
            for usersocialauth in UserSocialAuth.objects.filter(provider=LINKEDIN_OVERRIDE_BACKEND_NAME):
                usersocialauth.provider = LINKEDIN_ORIGINAL_BACKEND_NAME
                usersocialauth.save()
    ...
    models = {
        ...
        'forum.facebookaccount': {
            'Meta': {'unique_together': "(('app', 'user'),)", 'object_name': 'FacebookAccount'},
            'access_token': ('django.db.models.fields.CharField', [], {'max_length': '250', 'null': 'True', 'blank': 'True'}),
            'access_token_expires_on': ('django.db.models.fields.DateTimeField', [], {'null': 'True', 'blank': 'True'}),
            ...
            'user': ('django.db.models.fields.related.ForeignKey', [], {'related_name': "'facebook_accounts'", 'to': "orm['forum.User']"})
        },
        'forum.flag': {
        ...
        'forum.linkedinaccount': {
            'Meta': {'unique_together': "(('app', 'user'),)", 'object_name': 'LinkedinAccount'},
            'access_token': ('django.db.models.fields.CharField', [], {'max_length': '250', 'null': 'True', 'blank': 'True'}),
            'access_token_expires_on': ('django.db.models.fields.DateTimeField', [], {'null': 'True', 'blank': 'True'}),
            'app': ('django.db.models.fields.CharField', [], {'max_length': '100'}),
            'email': ('django.db.models.fields.CharField', [], {'max_length': '255', 'null': 'True', 'blank': 'True'}),
            ...
            'uid': ('django.db.models.fields.CharField', [], {'max_length': '100', 'null': 'True', 'blank': 'True'}),
            'user': ('django.db.models.fields.related.ForeignKey', [], {'related_name': "'linkedin_accounts'", 'to': "orm['forum.User']"})
        },
        'forum.markedtag': {
        ...
        'forum.questionsubscription': {
            ...
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'last_view': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2013, 7, 12, 0, 0)'}),
        ...
        'forum.user': {
            ...
            'notifications': ('django.db.models.fields.CharField', [], {'max_length': '1', 'null': 'True', 'blank': 'True'}),
        ...
        'forum.validationhash': {
            ...
            'expiration': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2013, 7, 13, 0, 0)'}),
